...
export default function RoomDetailView({ roomId, admin }: { roomId: string; admin: boolean }) {
  ...
  if (trackedId !== roomId) {
    setTrackedId(roomId);
    setRoom((!isFirebaseConfigured ? findRoomById(roomId) : undefined) ?? "loading");
  }

  useEffect(() => {
    if (!isFirebaseConfigured && findRoomById(roomId)) return;
    getRoomById(roomId)
        ...
        <div className="mb-3 flex items-center justify-between gap-2">
          <button
            ...
            onClick={() => {
              ...
              // direct link (no history). Fall back to a sensible home so the
              // button always does something — the admin rooms list in admin
              // mode, otherwise the public explore page.
              if (typeof window !== "undefined" && window.history.length > 1) {
                router.back();
              } else {
                router.push(admin ? "/user/admin" : "/explore");
              }
            }}
